# Torbox — User Account Service

Manage user account, tokens, device auth, referrals, subscriptions, and search engines.
...
**Example Response:**
```json
{ "success": true, "data": "NEW_TOKEN_STRING" }
```

---

## Device Authentication (OAuth for TV/CLI)

Used for TV apps or CLI tools to log in without entering passwords.
...
curl -H "Authorization: Bearer YOUR_TOKEN" \
  https://api.torbox.app/v1/api/user/getconfirmation
...
curl -H "Authorization: Bearer YOUR_TOKEN" \
  "https://api.torbox.app/v1/api/user/settings/searchengines"
...
# Enable / disable a search engine
curl -X POST -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
...
**Control operations:** `enable`, `disable`
